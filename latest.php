		...
		foreach($rows as $i => $row)
		{
			...
			switch($row['type'])
			{
				...
				case 5: // Quest
					...
					break;
				case 6: // Spell
					$comments[$i]['subject'] = $DB->selectCell('SELECT spellname FROM ?_spell WHERE spellID=?d LIMIT 1', $row['typeID']);
					break;
				case 7: // Zone
					// TODO
					break;
				case 8: // Faction
					$comments[$i]['subject'] = $DB->selectCell('SELECT name FROM ?_factions WHERE factionID=?d LIMIT 1', $row['typeID']);
					break;
				default:
